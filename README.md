...
- 加入有限的一次性工业、税制、选举与帝国制度事件，不使用长期循环事件池。常规临时修正通常持续 6–8 年；由地区建设触发的四项 `BYZ` 西方整合会议提供彼此独立的永久制度选择。
- “西方罗马公民权章程”的三条路线只为现有西方整合白名单中的未整合州提供 `+75%`、`+50%` 或 `+25%` 地区整合速度；事件后新取得的白名单州会同步获得，完成整合、失去领土或不再是 `BYZ` 后移除。
